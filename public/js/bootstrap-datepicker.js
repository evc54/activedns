...
!function ( $ ) {
  ...
  DatePicker.prototype = {
    ...
    , selectMonth: function(date) {
        ...
        if (!this.curMonth || !(this.curMonth.getFullYear() == newMonth.getFullYear() &&
          ...
          for (var ii = 0; ii <= num_days; ii++) {
			...
			var standard = '' + thisDay.getFullYear() + '-' + (thisDay.getMonth() < 9 ? '0' : '') + (thisDay.getMonth() + 1) + '-' + (thisDay.getDate() < 10 ? '0' : '') + thisDay.getDate();
            var $day = $('<div>').attr('date', this.format(thisDay)).attr('rel',standard);
          ...
          $('div', this.$days).click($.proxy(function(e) {
            ...
            this.update($targ.attr("date"),$targ.attr('rel'));
    ...
    , update: function(d,s) {
        this.$el.val(d).attr('rel',s).change();
      }
